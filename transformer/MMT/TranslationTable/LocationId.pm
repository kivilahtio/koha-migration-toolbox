...
sub statisticsLibrary($s, $kohaObject, $voyagerObject, $builder, $originalValue, $tableParams, $transParams) {
  return branchLoc(@_) unless (ref($kohaObject) eq 'MMT::Voyager2Koha::Item' || ref($kohaObject) eq 'MMT::Voyager2Koha::Holding'); #Fall back to the standard translator when necessary.
